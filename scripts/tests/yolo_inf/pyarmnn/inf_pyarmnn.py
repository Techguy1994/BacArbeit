...
model_path = "yolov5l-fp32.tflite"
#image_path = "dog.jpg"
...
print("Output tensors: ", len(output_tensors))
...
np.save("input_pyarmnn", image_data)
...
np.save("output_pyarmnn", output_data)
print(output_data)
sys.exit("End")
...
with open('pyarmnn.txt', 'w') as f:
    for out in output_data[0,:]:
...
#with open('tf_first_found_pyarmnn.txt', 'w') as f:
#    f.writelines(str(20248) + "\n")
#    for out in output_data[20248,:]:
#        f.writelines(str(out) + "\n")
...
cv2.imwrite("output_pyarmnn.jpg", output_img)