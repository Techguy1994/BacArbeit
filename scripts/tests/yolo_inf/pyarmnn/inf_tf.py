...
model_path = "yolo_v3_tiny_darknet_fp32.tflite"
#image_path = "dog.jpg"
...
for i in range(len(scores)):
    ...
    if ((scores[i] > 0.7) and (scores[i] <= 1.0)):
        ...
        with open('tf_first_found.txt', 'w') as f:
            f.writelines(str(i) + "\n")
            for out in output_data[i,:]:
                f.writelines(str(out) + "\n")

        
        print(i, labels[classes[i]],classes[i], scores[i])
        #H = frame.shape[0]
        #W = frame.shape[1]
