...
def inf_pyarmnn(model_path, iterations=50):
    ...
    print("\n")
    print(profiler_data[0])
    print("\n")

    for key, value in profiler_data[1].items():
        print(f"{key}: {value}")
    ...
    print(f"Total inference time (ms): {mean_total_time}"
    )
    ...
    for key, val in profiler_data.per_workload_execution_data.items():
        layer_inference_times = val['execution_time']
        mean_layer_time = np.mean(layer_inference_times) / 1000
        layer_time_backends.append((key, mean_layer_time, val['backend'])) 
        print(f"Layer: {key}, Time (ms): {mean_layer_time}, Backend: {val['backend']}")
    """
