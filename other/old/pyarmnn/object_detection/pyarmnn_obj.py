...
def draw_rect(output_data, width, height, img):
...
def inf_pyarmnn(model_path, iterations=50):
    ...
    for i in range(iterations):
        ...
        # result output format: [Locations, classes, scores, number of Detections]
        results = ann.workload_tensors_to_ndarray(output_tensors) # gather inference results into dict

        draw_rect(results, width, height, image)
