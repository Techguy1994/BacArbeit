...
def draw_rect(output_data, input_shape):
...
def inf_tflite(iter=10, n_big=5):
    ...
    #https://github.com/kmfrick/TFLite_DeepLabv3_Inference/blob/master/inference_pascal.py
    #https://colab.research.google.com/github/sayakpaul/Adventures-in-TensorFlow-Lite/blob/master/DeepLabV3/DeepLab_TFLite_COCO.ipynb#scrollTo=k3kAmscsedPH


    # Load the TFLite model and allocate tensors.
    interpreter = tflite.Interpreter(model_path="lite-model_deeplabv3_1_metadata_2.tflite")
    ...
    for i in range(iter):
        ...
        input_data = np.array(np.random.random_sample(input_shape), dtype=input_type)
        ...
        print()
        print(np.squeeze(interpreter.get_tensor(output_details[0]['index']).shape))
        print(np.squeeze(interpreter.get_tensor(output_details[0]['index'])))
        print()
        ...
        #draw_rect(output_data, input_shape)
