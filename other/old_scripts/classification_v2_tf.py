...
def return_n_biggest_result_tflite_runtime(output_data, output_details, class_dir, n_big=10):
...
def preprocess_image_tflite_mobilenet2(image_path, height, width, channels=3, data_type=np.float32):
    image = Image.open(image_path)
    image = image.resize((width, height), Image.LANCZOS)
...
def tflite_runtime(model_dir, img_dir, label_dir, n_big, niter):
    ...
    for i in range(niter):
        for img in img_dir:
            print("Image: ", img)
            img = preprocess_image_tflite_mobilenet2(img, input_shape[1], input_shape[2], data_type=input_type)
