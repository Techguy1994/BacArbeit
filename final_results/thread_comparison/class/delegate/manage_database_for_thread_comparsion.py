...
def manage_database(args):
    ...
    if df.empty:
        print("empty")
        df = add_entry(model_name, inference_type, api, is_only_lat, os, file_path, df, thread)
        return df
    else:
        ...
        ind = df.index[(df["model_name"] == model_name) & (df["os"] == os) & (df["api"] == api) & (df["thread count"] == str(thread))]
        print(ind)
        if len(ind) == 0:
            print("no row found")
            df = add_entry(model_name, inference_type, api, is_only_lat, os, file_path, df, thread)
...
def add_entry(model_name, inference_type, api, onyl_lat, os, file_path, df, thread):
    if thread == "False":
        entry = {"model_name": [model_name], "api": [api], "inference type": [inference_type], "os": [os], "thread count": [str(thread)]}
    else:
